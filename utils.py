...
def batch_export(root, out_dir="out", configs=None):
    ...
    sheet2 = workbook.add_sheet("直径分布")  # 在工作簿中新建一个表格
    if configs.out_list == "ALL":
        sheet.write(0, 0, "文件名")
        sheet.write(0, 1, "平均直径um")
        sheet.write(0, 2, "平均直径(像素)")
        sheet.write(0, 3, "直径cv")
        sheet.write(0, 4, "孔隙率")
        sheet.write(0, 5, "孔隙cv")
        sheet2.write(0, 0, "直径值um")
    else:
        pass
    i = 1
    for file_path in file_dir:
        ...
        diameter, diameter_cv, porosity, porosity_cv, D = reults_dic["diameter"],reults_dic["diameter_cv"],reults_dic["porosity"],reults_dic["porosity_cv"],reults_dic["D"]
        ...
        for m in range(len(D)):
            sheet2.write(m+1,0,D[m])
        print("###" * 10)
        print(file_path, "平均直径um:", real_diameter)
...
def sem_analysis(img):
    ...
    reults_dic = {"diameter":diameter, "diameter_cv":diameter_cv, "porosity":porosity, "porosity_cv":porosity_cv,"D": D}
